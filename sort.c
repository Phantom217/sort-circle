...
static void emit_u32le(unsigned long v, FILE *f) {
    fputc((v >> 0) & 0xff, f);
    fputc((v >> 8) & 0xff, f);
    fputc((v >> 16) & 0xff, f);
    fputc((v >> 24) & 0xff, f);
...
static void emit_u32be(unsigned long v, FILE *f) {
    fputc((v >> 24) & 0xff, f);
    fputc((v >> 16) & 0xff, f);
    fputc((v >> 8) & 0xff, f);
    fputc((v >> 0) & 0xff, f);
...
static void emit_u16le(unsigned v, FILE *f) {
    fputc((v >> 0) & 0xff, f);
    fputc((v >> 8) & 0xff, f);
}
...
static void rgb_split(unsigned long c, float *r, float *g, float *b) {
    *r = ((c >> 16) / 255.0f);
    *g = (((c >> 8) & 0xff) / 255.0f);
    *b = ((c & 0xff) / 255.0f);
}
...
static void ppm_write(const unsigned char *buf, FILE *f) {
    fprintf(f, "P6\n%d %d\n255\n", S, S);
    fwrite(buf, S * 3, S, f);
    fflush(f);
}
...
/*
 * writes a color to a particular pixel in the
 * buffer (assumed to be `s` by `s` pixels)
 */
static void ppm_set(unsigned char *buf, int x, int y, unsigned long color) {
    buf[y * S * 3 + x * 3 + 0] = color >> 16;
    buf[y * S * 3 + x * 3 + 1] = color >> 8;
    buf[y * S * 3 + x * 3 + 2] = color >> 0;
}

/* gets a color from a particular pixel in the buffer */
static unsigned long ppm_get(unsigned char *buf, int x, int y) {
...
static void ppm_dot(unsigned char *buf, float x, float y, unsigned long fgc) {
    /* extract color components */
    float fr, fg, fb;
    rgb_split(fgc, &fr, &fg, &fb);

    /* Determine the range of pixels over which the dot will be drawn.
    ...
    for (int py = miny; py <= maxy; py++) {
        ...
        for (int px = minx; px <= maxx; px++) {
            float dx = px - x;
            /* use the x and y distances to compute the distanceand smoothstep
             * value, which will be the alpha. Within the inner radius the color
             * is on 100%. Outside the outer radius it's 0%. Elsewhere it's
             * somewhere in between. */
            float d = sqrtf(dy * dy + dx * dx);
            float a = smoothstep(R1, R0, d);

            /* get the background color, and extract it's components */
            unsigned long bgc = ppm_get(buf, px, py);
            float br, bg, bb;
            rgb_split(bgc, &br, &bg, &bb);

            /* blend the foreground and background according to the computed
             * alpha value. Finally write the pixel back into the buffer. */
            float r = a * fr + (1 - a) * br;
            float g = a * fg + (1 - a) * bg;
            float b = a * fb + (1 - a) * bb;
...
static void ppm_char(unsigned char *buf, int c, int x, int y,
                     unsigned long fgc) {
    float fr, fg, fb;
    rgb_split(fgc, &fr, &fg, &fb);
...
static unsigned long hue(int v) {
...
static void frame(void) {
    ...
    /* Reset the swap counter for the next frame */
    memset(swaps, 0, sizeof(swaps));
}

/*
 * Generate and mix tones for each element that was swapped when producint a
 * frame.
 */
static void swap(int a[N], int i, int j) {
    int tmp = a[i];
    a[i] = a[j];
    a[j] = tmp;
    swaps[(a - array) + i]++;
...
static void sort_bubble(int array[N]) {
    int c;
    do {
        c = 0;
        for (int i = 1; i < N; i++) {
...
static void sort_odd_even(int array[N]) {
    int c;
    do {
        c = 0;
...
static void sort_insertion(int array[N]) {
    for (int i = 1; i < N; i++) {
...
static void sort_stoogesort(int *array, int i, int j) {
    static int c = 0;
    if (array[i] > array[j]) {
        swap(array, i, j);
        if (c++ % 32 == 0) {
...
static void sort_quicksort(int *array, int n) {
    if (n > 1) {
        int high = n;
        for (int i = 1; i < high;) {
            if (array[0] < array[i]) {
...
static int digit(int v, int b, int d) {
    for (int i = 0; i < d; i++) {
        v /= b;
    }
    return v % b;
...
static void sort_radix_lsd(int array[N], int b) {
    int c, total = 1;
    for (int d = 0; total; d++) {
        total = -1;
...
static void shuffle(int array[N], uint64_t *rng, unsigned flags) {
...
static const char *const sort_names[] = {
    [SORT_ODD_EVEN] = "Odd-even",   [SORT_BUBBLE] = "Bubble",
    [SORT_INSERTION] = "Insertion", [SORT_STOOGESORT] = "Stoogesort",
    [SORT_QUICKSORT] = "Quicksort", [SORT_RADIX_8_LSD] = "Radix LSD (base 8)",
};

static void run_sort(enum sort type) {
...
static FILE *wav_init(const char *file) {
